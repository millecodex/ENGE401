...
\begin{tasks}(2)
\task[]\example  \medskip\\Sketch $y =\cos  ( -t)$\\ 
Notice this reflection is on top of the original. Cosine is symmetric about the $y$-axis.\\
\includegraphics[width=7cm]{trigTrans1}\\

\task[]\example \medskip\\Sketch $y =\sin(x+\frac{\pi}{4})$\\
The angle is shifted to the left by 45 degrees ($\frac{\pi}{4}$ radians).\\
\includegraphics[width=7cm]{trigTrans2}

\task[]\example \medskip\\Sketch $y =\cos(2x)$\\
Here there are 2 complete cycles between 0 and $2\pi$.\\
\includegraphics[width=7cm]{trigTrans3}

\task[]\example \medskip\\Sketch $y =2\sin(0.5x)$\\
Two transformations applied together. The 2 refers to the amplitude.\\
\includegraphics[width=7cm]{trigTrans4}
\end{tasks}
