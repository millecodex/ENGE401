...
\begin{multicols}{2}
	\begin{center}
		\begin{tikzpicture}\pgfplotsset{ticks=none}
			...
			\end{axis}
		\end{tikzpicture}\end{center}\captionof*{tikzpicture}{When $0<a<1$, $y=a^{x}$ `decays' as $x$ gets big}\columnbreak
	\begin{center}
		\begin{tikzpicture}\pgfplotsset{ticks=none}
			\begin{axis}[
				scale=0.8,
				...
				\addplot [->,domain=-5:3.2,thick, samples=100, black] {2^x};
...
% multicols is so frustrating!
\columnbreak{Method 2: Take logarithms to the base $e$ \begin{align*}\ln  3^{x} &  = \ln  5 \\
x \ln  3 &  = \ln  5 \\
x &  = \frac{\ln  5}{\ln  3} \\
&  \approx 1.46497\end{align*}}
\end{multicols}
