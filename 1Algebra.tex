...
When solving an equation the order of operations is important. The acronym \textbf{BEDMAS} is used for simplifying expressions starting with B=brackets, Exponents, Division, Multiplication, Addition, and ending with S=subtraction. The \textit{reverse} is true for solving equations. First you must undo any subtraction or addition to isolate the variable.
...
\solution See the graph above. The domain is all the $x$-values that are included; this is everything between $-2$ and $+2$ inclusive. \medskip\\
