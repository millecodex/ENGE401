...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% \section{Exercises} %included in the \exheader
 \exheader
 \textbf{Section \ref{sec:introAlgebra} \nameref{sec:introAlgebra}}
 \begin{multicols}{2}
 \begin{exercises}
 	\begin{subquestions}[Remove the brackets\\]
 	\subquestion{$ -\left (x +y\right )$ }{$ -x -y$}\\
 	\subquestion{$ -3 (5 x -2 y)$}{$ -15 x +6 y$}
 \end{subquestions}
\begin{subquestions}[Calculate the value of\\]
	\subquestion{$\left (15.3\right )^{0}$}{$1$}\\
	\subquestion{$10^{ -2}$}{$0.01$}
\end{subquestions}
\begin{subquestions}[Simplify\\]
	\subquestion{$\left (3 a^{2} b\right )^{2}$}{$9 a^{4} b^{2}$}\\
	\subquestion{$\left(\frac{x}{3}\right)^{3} x^{3}$}{$\frac{x^{6}}{27}$}
\end{subquestions}
\begin{subquestions}[Evaluate\\]
	\subquestion{$\sqrt[{4}]{2.7}$ accurate to 2 decimal places. }{$1.28$}\\
	\subquestion{b}{b}
\end{subquestions}
\begin{subquestions}[Remove the brackets and simplify \\]
	\subquestion{$\left (x^{2} +5 x -1\right ) -\left (2 x -3\right )$}{$x^{2} +3 x +2$}\\
	\subquestion{$\left (2 x -1\right ) \left (2 x +1\right )$}{$4 x^{2} -1$}
\end{subquestions}
\begin{subquestions}[Factorise the expressions\\]
	\subquestion{$x^{2} +11 x +28$}{$\left (x +7\right ) \left (x +4\right )$}\\
	\subquestion{$2 x^{2} -5 x -12$}{$\left (2 x +3\right ) \left (x -4\right )$}
\end{subquestions}
\question{Simplify $\frac{x^{2} +5 x +6}{x^{2} +2 x -3}$ by factorising and then cancelling }{$\frac{x +2}{x -1}$}
\begin{subquestions}[Solve the equations\\]
	\subquestion{$7 x -16 =\frac{2}{3} x +4$ }{$3.16$}\\
	\subquestion{$\left (x -2\right )^{2} =15$}{$ -1.87$ or $5.87$}\\
	\subquestion{$x^{2} -2 x -8 =0$}{$4$ or $ -2$}\\
	\subquestion{$2 x^{2} +5 x -4 =0$}{$0.637$ or $ -3.137$}
\end{subquestions}
\end{exercises}
\end{multicols}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\textbf{Section \ref{sec:functions} \nameref{sec:functions}}
\begin{multicols}{2}
	\begin{exercises}	
	\begin{subquestions}[Make $t$ the subject of the equation \\]
		\subquestion{$v =u +a t$}{$t =\frac{v -u}{a}$ }\\
		\subquestion{$l =l_{0} \left (1 +\alpha  t\right )$}{$t =\frac{1}{\alpha } \left (\frac{l}{l_{0}} -1\right )$}
	\end{subquestions}
	\begin{subquestions}[If $f (x) =x^{2}$ and $g (x) =x^{3}$ evaluate\\]
	\subquestion{$f (2)$}{$4$}\\
	\subquestion{$f ( -2)$}{$4$}\\
	\subquestion{$g (3)$}{$27$}\\
	\subquestion{$g ( -3)$}{$-27$}
...
\question{The volume of a pipe with length $l$, inner radius $r$ and outer radius $R$ is $V =\pi  \left (R^{2} -r^{2}\right ) l\text{.}$ Find the volume when $R =3.1 \mbox{m}$, $r =2.2 \mbox{m}$ and $l =5.3 \mbox{m}\text{.}$}{$79.4 \mathrm{m}^{3}$}
	\begin{subquestions}[Sketch the following graphs (without using Desmos).\\]
		\subquestion{$y=4x-4$}{b}\\
		\subquestion{$2y=\frac{x}{3}+1$}{b}\\
		\subquestion{$y =x^{2} -3$ }{b}\\
		\subquestion{$y =-(x+1)^{2}$ }{b}
	\end{subquestions}
\question{ Find the distance between the points $\left (2 , -3\right )$ and $\left (5 , -1\right )\text{.}$}{$3.61$}
\question{Find the $x$ and $y$ intercepts for the graph of $y =x^{2} -3$}{$x-$intercepts $1.73$ and $ -1.73$ and $y-$intercept $ -3$}
	\begin{subquestions}[Use the grid provided to answer the following questions. **INSERT GRID HERE**\\]
		\subquestion{Show the points $A \left (3 ,5\right )$ and $B \left ( -2 , -5\right )$ on the graph.}{a}%\includegraphics[ width=2.0496in, height=2.8669in,]{L4SZ2709}}\\
		\subquestion{Calculate the slope of the line through $A$ and $B$. i.e. the slope of $\bar{AB}\text{.}$}{$2$}\\
		\subquestion{What is the equation of the line $A B$?}{$y =2 x -1$}\\
		\subquestion{What is the equation of the line parallel to the line $A B$ through the point $\left ( -2 ,3\right )\text{?}$ }{$y =2 x +7$}
		\subquestion{Starting from the point $B$ on the graph frame draw a line with a slope of $\frac{4}{5}\text{.}$ }{arg2}
	\end{subquestions}
%	\question{Complete the table of values for the function $g (x) =x^{3} -x$ and sketch the graph\vspace{0.5cm} \\\relax
...
\end{multicols}
	...
	%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	\textbf{Section \ref{sec:polynomials} \nameref{sec:polynomials}}
	\begin{multicols}{2}
		\begin{exercises}	
			\begin{subquestions}[What is the degree of the polynomial?\\]
				\subquestion{$y=x^2$}{$2$}\\
				\subquestion{$4x^3-2x^2+6=0$}{$3$}\\
				\subquestion{$a^{99}-99a^2=-99$}{$99$}
			\end{subquestions}
	\end{exercises}
\end{multicols}	
	
	%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\textbf{Section \ref{sec:systemsOfEquations} \nameref{sec:systemsOfEquations}}
\begin{multicols}{2}
	\begin{exercises}	
		\begin{subquestions}[l\\]
			\subquestion{plot something}{\rule{150pt}{100pt}}\\
			\subquestion{a}{a}%{\includegraphics[width=2cm]{poly}}}
			\subquestion{a}{a}
		\end{subquestions}
	\end{exercises}
\end{multicols}	
